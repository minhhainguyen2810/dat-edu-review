...
async function sendComment(body: Inputs, workplaceId: string) {
  const res = await fetch(`/api/workplace/comment`, {
    method: 'POST',
    body: JSON.stringify({ ...body, workplace_id: workplaceId })
  });
...
type WorkDetailProps = Omit<Database['workplace'], 'id'> & {
  id: number;
};
...
export default function CreateComment({
  ...
  workplaceDetail: WorkDetailProps;
}) {
  const { session } = useUserInfo();
  const { register, handleSubmit, control } = useForm<Inputs>();
  const searchParams = useParams<{ workplaceId: string }>();
  const { workplaceId = '' } = searchParams || {};
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="space-y-12">
          <div className="border-b border-gray-900/10 pb-12">
            <Title>Viết đánh giá cho khoa {workplaceDetail.name}</Title>
