...
interface WorkplaceCommentProps {
  workplaceComments: (Pick<
    Database['workplace_comment'],
    'rate_overall' | 'user' | 'need_to_improved' | 'date'
  > & {
    id: number;
  })[];
}

export default function WorkplaceComment({
  workplaceComments
}: WorkplaceCommentProps) {
  if (!workplaceComments.length) {
  ...
  return (
    <Card className="my-6 space-y-4">
      <ReactImageGallery
        ...
        // renderItem={() => null}
      />
      {workplaceComments.map((comment, index) => (
